...
parser.add_argument(
    "-f", type=str, dest="out_file", required=False, help="Output context file"
)
parser.add_argument(
    "-c",
...
with tempfile.NamedTemporaryFile(delete=True, suffix=suffix) as tmp_file:
    ...
    try:
        ...
        if args.verbose:
            ...
            eprint("Use -v for more verbose error output")
        exit(1)

lines = ctx.splitlines(True)
for i in reversed(range(len(lines))):
    if lines[i].startswith("#define __cplusplus"):
        lines.pop(i)
    elif lines[i].startswith("#define __STDC_HOSTED__"):
        lines.pop(i)
    elif lines[i].startswith("#define __STDC__"):
        lines.pop(i)
    elif lines[i].startswith("#define __STDC_VERSION__"):
        lines.pop(i)
